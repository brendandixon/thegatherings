class MembershipAuthorizer < ApplicationAuthorizer
  ...
  def creatable_by?(member, options = {})
    super
    ((to_community? || to_campus?) && (for_self?(member) || is_overseer?)) ||
    (to_gathering? && (is_gathering_overseer? || is_coach?))
  end

  def readable_by?(member, options = {})
    super
    for_self?(member) ||
    ((to_community? || to_campus?) && is_overseer?) ||
    (to_gathering? && (is_gathering_participant? || is_gathering_overseer? || is_coach?))
  end
